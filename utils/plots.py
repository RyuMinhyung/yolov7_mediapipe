...
def output_to_keypoint(output, ori_imgsz, imgsz):
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf]
    ...
    keypoint_output = np.array(targets)

    if len(keypoint_output) == 0:
        return None, None
    
    # Drop
    keypoint_output = keypoint_output[:, 7:]

    # Resize boxes to the original image size
    keypoint_output[:, 0::3] *= ori_imgsz[1] / imgsz[1] # 가로
    keypoint_output[:, 1::3] *= ori_imgsz[0] / imgsz[0] # 세로
    
    return [o], keypoint_output
